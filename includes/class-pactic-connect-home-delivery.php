...
class Pactic_Connect_Home_Delivery extends WC_Shipping_Method {
	
	private $shipping_cost = 0;
	
	public function __construct( $instance_id = 0 ) {
		  
		$this->id = 'pactic_connect_home_delivery';
		$this->instance_id = absint( $instance_id );
		$this->method_title = __( 'Pactic home delivery', 'pactic-connect' );
		$this->method_description = __( 'Pactic home delivery', 'pactic-connect' );
		$this->enabled = "yes";
	...
	public function init_form_fields() {
		...
		$this->instance_form_fields = array(
			'title' => array(
				...
				'default'     => __( 'Pactic home delivery', 'pactic-connect' ),
				'desc_tip'    => true,
			),
			...
			'service' => array(
				...
				'options'     => $this->get_home_delivery_services(),
			),
			...
			'detailed_pricing' => array(
				...
				'custom_attributes'  => array('instance_id' => $this->instance_id, 'method_name' => 'home_delivery' ),
	...
	public function get_home_delivery_services(){
		
		$home_delivery_settings = get_option('pactic_connect__home_delivery_settings');

		$home_delivery_services = array();
		
		if( !empty( $home_delivery_settings ) ){
		
			foreach ( $home_delivery_settings as $home_delivery_setting_id => $home_delivery_setting ){
				
				if( 'yes' == $home_delivery_setting['enable'] ){
					
					$home_delivery_services[ $home_delivery_setting['service_id']  ] = $home_delivery_setting['name'];
	...
	public function get_cost( $instance_id = 0 ) {
		...
		$instance_settings = get_option('woocommerce_pactic_connect_home_delivery_'.$instance_id.'_settings');
